		...
		)
{
	...
	// TODO use a sandbox, see gtk2_ardour/luainstance.cc
	...
	if (lua_factory.isFunction()) {
		...
		(*_lua_add)(name, lua_factory, tbl_arg); // throws luabridge::LuaException
		set_dirty();
	} else {
		throw SessionException (_("Lua script does not define a factory method"));
	}
}
...
Session::try_run_lua ()
{
	...
	if (tm.locked ()) {
		try { (*_lua_run)(); } catch (luabridge::LuaException const& e) { }
	}
}
...
Session::setup_lua ()
{
	...
	lua.do_command (
		  ...
		  "          list = list, restore = restore, save = save}"
			" end"
			" "
			" sess = ArdourSession ()"
			" ArdourSession = nil"
	...
	if (!_lua_run || !_lua_add || !_lua_del || !_lua_list || !_lua_save || !_lua_load) {
		fatal << string_compose (_("programming error: %1"),
				X_("Failed to setup Lua interpreter"))
			<< endmsg;
		abort(); /*NOTREACHED*/
